...
class PennerBot:
    ...
    def _trigger_auto_sell_check(self, current_price_cents: int):
        ...
        try:
            ...
            if result.get("success"):
                self.log(
                    f"[OK] Auto-Sell: EUR {result.get('earned', '0')} erwirtschaftet"
                )
            else:
    ...
    def _save_money(self, html: str):
        ...
        try:
            with get_session() as s:
                last_entry = (
                    s.query(MoneyHistory).order_by(MoneyHistory.id.desc()).first()
                )
    ...
    def _save_rank(self, rank: int):
        ...
        try:
            with get_session() as s:
                last_entry = (
                    s.query(RankHistory).order_by(RankHistory.id.desc()).first()
                )
                ...
                with get_session() as s:
                    ...
                    s.add(
                        Settings(
                            ...
                            value=CredentialEncryption.encrypt(password),
                        )
                    )
            ...
            if response.status_code == 200:
                if "Es läuft bereits eine Weiterbildung" in response.text:
                    ...
                    try:
                        ...
                        self.log(f"[WARN] Could not parse counters: {e}")
                    return {
                        "success": True,
    ...
    def sober_up_with_food(self, target_promille: float = 0.0):
        try:
            ...
            for food_item in available_food:
                ...
                food_effect = food_item.get("effect", 0.0)
            ...
            if total_ate:
                ...
                self.log(
                    f"[INFO] Auto-Essen erfolgreich: {food_str} -> {current_promille:.2f}‰"
                )
                return {
                    "success": True,
    ...
    def _save_activity_state(
        ...
        try:
            with get_session() as s:
                existing = (
                    s.query(BotActivity).filter_by(activity_type=activity_type).first()
                )
                if is_running:
                    if existing:
                        ...
                        if seconds_remaining and seconds_remaining > 0:
                            existing.expected_end_time = datetime.now() + timedelta(
                        ...
                        if seconds_remaining and seconds_remaining > 0:
                            new_activity.expected_end_time = datetime.now() + timedelta(
                                seconds=seconds_remaining
                            )
                        s.add(new_activity)
                elif existing:
            ...
            if "locked" not in str(e).lower():
                self.log(
                    f"[WARN] Failed to save activity state for {activity_type}: {e}"
                )
        ...
        try:
            with get_session() as s:
                ...
                if not saved_activities:
                    ...
                    return
                self.log(
                    f"[RESTORE] Found {len(saved_activities)} running activities in database"
                )
                for activity in saved_activities:
                    ...
                    if (
                        activity.expected_end_time
                        and datetime.now() > activity.expected_end_time
                    ):
                    ...
                    if activity_type == "skill":
                        if seconds_remaining and seconds_remaining > 0:
                            ...
                            self.log(
                                f"[RESTORE] Skill training saved: {activity_subtype}, {seconds_remaining}s remaining"
                            )
                        ...
                        if seconds_remaining and seconds_remaining > 0:
                            ...
                            self.log(
                                f"[RESTORE] Fight saved: {seconds_remaining}s remaining"
                            )
                        ...
                        if seconds_remaining and seconds_remaining > 0:
                            ...
                            self.log(
                                f"[RESTORE] Bottle collecting saved: {seconds_remaining}s remaining"
                            )
                expired_activities = (
                    s.query(BotActivity)
                    .filter(BotActivity.expected_end_time < datetime.now())
                    .all()
                )
                for expired in expired_activities:
                    self.log(
                        f"[CLEAN] Cleaning up expired activity: {expired.activity_type}"
                    )
                    s.delete(expired)
        except Exception as e:
            if "locked" not in str(e).lower():
    ...
    def _restore_interrupted_workflows(self):
        try:
            ...
            self._restored_skill_subtype = None
            self._restored_fight_running = self.fight_running
            self._restored_bottles_running = self.bottles_running
            self.log(
                f"[RESTORE] Before verification - skill={self.skill_running}, fight={self.fight_running}, bottles={self.bottles_running}"
            )
            if self.skill_running or self.fight_running or self.bottles_running:
                self.log(
                    "[SYNC] Verifying restored activity states with game server..."
                )
                if self.refresh_status(force=True):
                    if not self.skill_running and self._restored_skill_running:
                        self._save_activity_state(
                            "skill", False, 0, self._restored_skill_subtype
                        )
                        self.log(
                            "[RESTORE] Skill was interrupted - will restart when training enabled"
                        )
                    ...
                    if not self.bottles_running and self._restored_bottles_running:
                        self._save_activity_state("bottles", False, 0)
                        self.log(
                            "[RESTORE] Bottle collecting was interrupted - will restart"
                        )
                    if self.skill_running and self._restored_skill_running:
                        self.log(
                            f"[RESTORE] Skill still running on game server - {self.skill_seconds_remaining}s remaining"
                        )
                    if self.fight_running and self._restored_fight_running:
                        self.log(
                            f"[RESTORE] Fight still running on game server - {self.fight_seconds_remaining}s remaining"
                        )
                    if self.bottles_running and self._restored_bottles_running:
                        self.log(
                            f"[RESTORE] Bottle collecting still running on game server - {self.bottles_seconds_remaining}s remaining"
                        )
                    if not self.skill_running and self._restored_skill_running:
                        self._save_activity_state(
                            "skill", False, 0, self._restored_skill_subtype
                    ...
                    if not self.bottles_running and self._restored_bottles_running:
                        ...
                        with get_session() as s:
                            existing = (
                                s.query(BotActivity)
                                .filter_by(activity_type="bottles")
                                .first()
                            )
                            if existing:
                                existing.was_interrupted = True
                        self.log(
                            "[RESTORE] Bottle collecting was interrupted - marked for restart"
                        )
                    if self.skill_running:
                        remaining = self.skill_seconds_remaining or 0
                        self.log(
                            f"[SYNC] Skill training active - {remaining}s remaining"
                        )
                    if self.fight_running:
                    ...
                    if self.bottles_running:
                        ...
                        self.log(
                            f"[SYNC] Bottle collecting active - {remaining}s remaining"
                        )
                else:
                    self.log(
                        "[WARN] Could not verify restored activities with game server"
                    )
        except Exception as e:
        ...
        try:
            ...
            with get_session() as s:
                ...
                if not self.refresh_status(force=True):
                    ...
                    return
                self.log(
                    f"[AUTO] Config check: bottles={config.bottles_enabled}, training={config.training_enabled}, fight={config.fight_enabled}, is_running={config.is_running}"
                )
                self.log(
                ...
                if should_start_training:
                    ...
                    if autodrink_enabled:
                        self.log(
                            f"[AUTO] Auto-Trinken aktiviert (Ziel: {target_promille:.1f}‰)..."
                        )
                        from .tasks import auto_drink_before_training

                        drink_result = auto_drink_before_training(
                            self, target_promille=target_promille
                        )
                        if drink_result.get("drank"):
                            self.log(
                                f"[AUTO] Auto-drink completed: {drink_result.get('message')}"
                            )
                        elif drink_result.get("success"):
                            self.log(
                                f"[AUTO] Auto-drink: {drink_result.get('message')}"
                            )
                    ...
                    if result.get("success"):
                        ...
                        if autodrink_enabled:
                            ...
                            try:
                                sober_result = self.sober_up_with_food(
                                    target_promille=0.0
                                )
                                if sober_result.get("ate"):
                                    self.log(
                                        f"[AUTO] Auto-Essen: {sober_result.get('message')}"
                                    )
                                else:
                                    self.log(
                                        f"[AUTO] Auto-Essen: {sober_result.get('message')}"
                ...
                if should_start_fight:
                    ...
                    result = start_fight(self)
                    if result.get("success"):
                        self.log("[AUTO] Fight started successfully")
                    else:
                        self.log(
                            f"[AUTO] Failed to start fight: {result.get('message')}"
                        )
        except Exception as e:
    ...
    def check_interrupted_activities(self) -> Dict[str, Any]:
        ...
        result = {"checked": False, "interrupted": []}
                ...
                if self.refresh_status(force=True):
                    for activity in saved_activities:
                        ...
                        if not is_running_on_server:
                            ...
                            self.log(
                                f"[CHECK] Activity {activity_type} was interrupted (manually canceled)"
                            )
        except Exception as e:
        ...
        try:
            with get_session() as s:
                ...
                if not config:
                    return result
                running_activities = (
                    s.query(BotActivity).filter_by(is_running=True).all()
                )
                activities_info = []
                for activity in running_activities:
                    ...
                    activity_info = {
                        ...
                        "expected_end": (
                            activity.expected_end_time.isoformat()
                            if activity.expected_end_time
                            else None
                        ),
                    }
                    activities_info.append(activity_info)
                interrupted_activities = (
                    s.query(BotActivity).filter_by(was_interrupted=True).all()
                )
                interrupted_info = []
