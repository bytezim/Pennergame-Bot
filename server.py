    ...
    while True:
        try:
            ...
            if activity_name == "bottles":
                ...
                logger.info(
                    "Training completed in game, checking if we need to reschedule..."
                )
                try:
                    with get_session() as s:
                        from src.models import BotConfig
                        ...
                        if config and config.is_running and config.training_enabled:
                            ...
                            logger.info(
                                "Rescheduled next training after game completion"
                            )
                        else:
                            logger.debug(
                                "Bot not running or training disabled, skipping reschedule"
                            )
                except Exception as e:
                    logger.error(f"Error rescheduling training after completion: {e}")
            elif activity_name == "fight":
                logger.info(
                    "Fight completed in game, checking if we need to reschedule..."
                )
                try:
                    with get_session() as s:
                        from src.models import BotConfig
...
async def lifespan(app: FastAPI):
    ...
    # Track initialization status for API endpoints
    ...
    def complete_initialization():
        try:
            ...
            event_handler_thread = threading.Thread(
                target=event_handler_worker, daemon=True
            )
    ...
    # Start all initialization in background thread
    init_thread = threading.Thread(target=complete_initialization, daemon=True)
    init_thread.start()

    # Yield immediately so app can start accepting requests
    yield
...
def get_bot_config():
    ...
    with get_session() as s:
        ...
        return {
            ...
            "fight_pause_minutes": config.fight_pause_minutes,
            "rotation_enabled": config.rotation_enabled,
            "rotation_start_with": config.rotation_start_with,
            "last_started": (
...
def update_bot_config(**kwargs):
    ...
    logger.info(f"update_bot_config called with kwargs: {kwargs}")
    ...
    if "fight_pause_minutes" in kwargs:
        pause = kwargs["fight_pause_minutes"]
        if pause < 1:
            kwargs["fight_pause_minutes"] = 1
        elif pause > 60:
            kwargs["fight_pause_minutes"] = 60
    if "rotation_start_with" in kwargs:
        start = kwargs["rotation_start_with"]
        if start not in ["fight", "bottles"]:
            kwargs["rotation_start_with"] = "bottles"
    with get_session() as s:
        config = s.query(BotConfig).first()
        if not config:
            config = BotConfig()
...
def _bot_collect_bottles_task():
    ...
    try:
        ...
        if not get_bot().is_logged_in(skip_log=True):
            ...
            return
        try:
            activities = get_bot().get_activities_data(use_cache=False)
            ...
            if bottles_status.get("collecting", False):
                ...
                if seconds_remaining > 0:
                    ...
                    get_bot().log(
                        f"⏳ Bottle collection already running, {seconds_remaining}s remaining (~{minutes_remaining}m)"
                    )
                    with get_session() as s:
                        from src.models import BotConfig
        ...
        if result.get("success"):
            ...
            try:
                ...
                current_bot = get_bot()
                bottles_seconds = getattr(
                    current_bot, "bottles_seconds_remaining", duration_minutes * 60
                )
...
def _schedule_next_bottles_task():
    ...
    get_bot().log(
        f"⏰ Next bottle collection in ~{actual_duration_minutes + actual_pause_minutes} minutes (collecting: {actual_duration_minutes}m + pause: {actual_pause_minutes}m)"
    )
    scheduler.add_job(
    ...
    try:
        ...
        if result.get("interrupted"):
            for activity_type in result["interrupted"]:
                current_bot.log(
                    f"[MONITOR] Detected interrupted {activity_type} - restarting..."
                )
                if activity_type == "bottles" and bottles_enabled:
                    current_bot.log(
                        "[MONITOR] Restarting interrupted bottle collection..."
                    )
                    from src.tasks import search_bottles
                    ...
                    with get_session() as s:
                        activity = (
                            s.query(BotActivity)
                            .filter_by(activity_type="skill", was_interrupted=True)
                            .first()
                        )
                        if activity and activity.activity_subtype:
                            subtype = activity.activity_subtype
                            current_bot.log(
                                f"[MONITOR] Restarting interrupted training: {subtype}"
                            )
...
def manage_scheduler_jobs(config):
    ...
    if not current_bot.is_logged_in(skip_log=True):
        ...
        if config["is_running"]:
            ...
            if not monitor_job:
                ...
                current_bot.log(
                    "[MONITOR] Activity monitor job scheduled"
                )
        return
    skill_already_running = (
        current_bot.skill_running
        if hasattr(current_bot, "skill_running") and current_bot.skill_running
        else False
    )
    bottles_already_running = (
        current_bot.bottles_running
        ...
        if config["rotation_enabled"]:
            if config["rotation_start_with"] == "bottles":
                ...
                if bottles_already_running:
                    ...
                    current_bot.log("Plane ersten Bottle-Job (Rotation start)...")
                    run_date = now
                    scheduler.add_job(
                        _bot_collect_bottles_task,
                        trigger="date",
                ...
                current_bot.log(
                    "[SKIP] Rotation enabled, starting with fight - skipping bottles"
                )
        else:
            bottle_job = scheduler.get_job(BOTTLE_JOB_ID)
            if bottles_already_running:
    ...
    if config["fight_enabled"] and config["is_running"]:
        if config["rotation_enabled"]:
            if config["rotation_start_with"] == "fight":
                fight_job = scheduler.get_job(FIGHT_JOB_ID)
                if current_bot.fight_running:
                    current_bot.log("[SKIP] Kampf bereits aktiv im Spiel")
                elif not fight_job:
                    current_bot.log("Plane ersten Kampf-Job (Rotation start)...")
                    run_date = now
                    scheduler.add_job(
                        _bot_fight_task,
                        trigger="date",
                ...
                current_bot.log(
                    "[SKIP] Rotation enabled, starting with bottles - skipping fight"
                )
        else:
            fight_job = scheduler.get_job(FIGHT_JOB_ID)
            if current_bot.fight_running:
                ...
                current_bot.log("Plane ersten Kampf-Job...")
                run_date = now
                scheduler.add_job(
                    _bot_fight_task,
                    trigger="date",
...
def _schedule_next_training_task(skip_training: bool = False):
    ...
    if actual_seconds_remaining > 0:
        get_bot().log(
            f"⏰ Next training in ~{actual_duration_minutes + actual_pause_minutes} minutes (training: {actual_duration_minutes}m + pause: {actual_pause_minutes}m)"
        )
    else:
        get_bot().log(
            f"⏰ Next training in ~{actual_pause_minutes} minutes (pause only)"
        )
    scheduler.add_job(
        _bot_training_task,
        trigger="date",
...
def health(request: Request) -> Dict[str, Any]:
    # During initialization, return a special status to indicate backend is starting
    if hasattr(request.app.state, 'initializing') and request.app.state.initializing:
        return {"ok": True, "status": "initializing", "version": "0.2.0"}
    elif hasattr(request.app.state, 'initialized') and request.app.state.initialized:
        return {"ok": True, "status": "ready", "version": "0.2.0"}
    else:
        # Default case - assume ready if we can't determine state
        return {"ok": True, "status": "ready", "version": "0.2.0"}


@app.get("/api/metrics/performance")
def performance_metrics() -> Dict[str, Any]:
    return {"performance": perf_monitor.get_stats(), "cache": get_cache_stats()}
...
def dashboard_status() -> Dict[str, Any]:
    try:
        ...
        return {
            ...
            "penner": penner_data,
            "activities": get_activity_status(current_bot),
    ...
    try:
        ...
        if current_bot is None:
            return {
                "logged_in": False,
                "penner": None,
                "activities": {"error": "Bot initializing"},
            }
        if getattr(current_bot, "logged_in", False):
            try:
                current_bot.refresh_status(force=False)
...
def auto_login() -> Dict[str, Any]:
    try:
        current_bot = get_bot()
        if getattr(current_bot, "logged_in", False):
            return {"success": True, "message": "Already logged in", "logged_in": True}
        with get_session() as s:
            username_setting = s.query(Settings).filter_by(key="username").first()
            password_setting = (
                s.query(Settings).filter_by(key="password_encrypted").first()
            )
...
def activities_upcoming() -> Dict[str, Any]:
    ...
    try:
        ...
        if bottles_running and bottles_remaining and bottles_remaining > 0:
            ...
            if (
                config.get("bottles_enabled")
                and config.get("bottles_pause_minutes", 1) > 0
            ):
        ...
        if skill_running and skill_remaining and skill_remaining > 0:
            ...
            if (
                config.get("training_enabled")
                and config.get("training_pause_minutes", 1) > 0
            ):
...
def update_config(payload: Dict[str, Any]) -> Dict[str, Any]:
    try:
        logger.info(f"Received config update payload: {payload}")
        update_bot_config(**payload)
        config = get_bot_config()
        logger.info(f"Config after update: {config}")
        manage_scheduler_jobs(config)
...
def bot_start() -> Dict[str, Any]:
    try:
        ...
        current_bot.log(
            f"[SCHEDULER] {len(existing_jobs)} Jobs aktiv, running={scheduler.running}"
        )
        current_bot.log(
            f"[CONFIG] bottles_enabled={config['bottles_enabled']}, training_enabled={config['training_enabled']}, fight_enabled={config['fight_enabled']}"
        )
        ...
        current_bot.log(
            f"[CONFIG] bottles={config['bottles_enabled']}, training={config['training_enabled']}, fight={config['fight_enabled']}"
        )
        ...
        if not is_logged_in:
            current_bot.log(
                "Nicht eingeloggt - Tasks werden bei Ausführung übersprungen"
            )
        ...
        if resume_info.get("has_interrupted_activities"):
            ...
            for activity in resume_info["interrupted_activities"]:
                ...
                if activity_type == "bottles" and config["bottles_enabled"]:
                    ...
                    if result.get("success"):
                        current_bot.log(
                            f"Restarted bottle collection: {result.get('message', '')}"
                        )
                        from src.models import BotActivity
                        ...
                        with get_session() as s:
                            interrupted = (
                                s.query(BotActivity)
                                .filter_by(
                                    activity_type="bottles", was_interrupted=True
                                )
                                .first()
                            )
                            if interrupted:
                        ...
                        current_bot.log(
                            f"Failed to restart bottle collection: {result.get('message', '')}"
                        )
                elif activity_type == "skill" and config["training_enabled"]:
                    subtype = activity.get("subtype")
                    if subtype:
                        current_bot.log(
                            f"[RESTART] Restarting interrupted skill training: {subtype}"
                        )
                        result = current_bot.start_skill(subtype)
                        if result.get("success"):
                            current_bot.log(
                                f"Restarted skill training: {result.get('message', '')}"
                            )
                            from src.models import BotActivity
                            ...
                            with get_session() as s:
                                interrupted = (
                                    s.query(BotActivity)
                                    .filter_by(
                                        activity_type="skill", was_interrupted=True
                                    )
                                    .first()
                                )
                                if interrupted:
        ...
        if config["bottles_enabled"]:
            try:
                ...
                if not bottles_status.get("collecting", False):
                    current_bot.log(
                        "[START] Bottle collecting not running, starting now..."
                    )
                    from src.tasks import search_bottles
                    ...
                    if result.get("success"):
                        current_bot.log(
                            f"Started bottle collection: {result.get('message', '')}"
                        )
                    else:
                        current_bot.log(
                            f"Failed to start bottle collection: {result.get('message', '')}"
                        )
            except Exception as e:
        ...
        if config["training_enabled"]:
            if not current_bot.skill_running:
                ...
                if autodrink:
                    ...
                    current_bot.log(
                        f"Auto-Trinken: {drink_res.get('message', '')} - Aktuell: {drink_res.get('current_promille', 0):.2f}‰"
                    )
                from src.tasks import start_training
                ...
                skill_type = (
                    training_skills[0]
                    if training_skills and isinstance(training_skills, list)
                    else "att"
                )
                ...
                if result.get("success"):
                    ...
                    if autodrink:
                        ...
                        current_bot.log(
                            f"Auto-Essen: {sober.get('message', '')} - Promille: {sober.get('current_promille', 0):.2f}‰"
                        )
                else:
                    current_bot.log(
                        f"Failed to start training: {result.get('message', '')}"
                    )
...
def _bot_fight_task():
    ...
    try:
        with get_session() as s:
            config = s.query(BotConfig).first()
            if not config or not config.is_running or (not config.fight_enabled):
                get_bot().log("⏭️ Fight task skipped: bot not running or disabled")
                return
        if not get_bot().is_logged_in(skip_log=True):
            get_bot().log("Fight task: not logged in")
            return
        try:
            activities = get_bot().get_activities_data(use_cache=False)
            fight_running = activities.get("fight", {}).get("running", False)
            if fight_running:
                seconds_remaining = activities.get("fight", {}).get(
                    "seconds_remaining", 0
                )
                if seconds_remaining > 0:
                    minutes_remaining = seconds_remaining // 60
                    get_bot().log(
                        f"⏳ Fight already running, {seconds_remaining}s remaining (~{minutes_remaining}m)"
                    )
                    with get_session() as s:
                        from src.models import BotConfig
        ...
        from src.tasks import start_fight
...
def _schedule_next_fight_task():
    ...
    try:
        bot = get_bot()
        if bot.fight_running and bot.fight_seconds_remaining:
            actual_seconds_remaining = bot.fight_seconds_remaining
        else:
            activities = bot.get_activities_data(use_cache=False)
            fight_status = activities.get("fight", {})
            if fight_status.get("running") and fight_status.get("seconds_remaining"):
                actual_seconds_remaining = fight_status.get("seconds_remaining", 0)
    except Exception as e:
        get_bot().log(f"Error getting fight timer: {e}")
        actual_seconds_remaining = 0
...
@app.post("/api/actions/fight/start")
def start_fight() -> Dict[str, Any]:
    try:
        from src.tasks import start_fight
        ...
        if not current_bot.logged_in:
            raise HTTPException(status_code=401, detail="Not logged in")
        result = start_fight(current_bot)
        if result.get("success"):
            invalidate_cache_pattern("dashboard")
            invalidate_cache_pattern("status")

            # Force a status refresh to update the UI
            try:
                current_bot.refresh_status(force=True)
            except Exception as e:
                logger.warning(f"Failed to refresh status after fight start: {e}")

            try:
                from src.events import emit_activity_started, emit_status_changed

                duration = getattr(get_bot(), "fight_seconds_remaining", 600)
                emit_activity_started("fight", duration)
                current_bot = get_bot()
                emit_status_changed(get_activity_status(current_bot))
            except Exception as e:
                logger.warning(f"Failed to emit events after fight start: {e}")
        return result
    except HTTPException:
        raise
    except Exception as e:
...
@app.get("/api/actions/fight/status")
def fight_status(force_refresh: bool = False) -> Dict[str, Any]:
    try:
        from src.tasks import get_fight_status

        current_bot = get_bot()
        if not current_bot.logged_in:
            raise HTTPException(status_code=401, detail="Not logged in")
        result = get_fight_status(current_bot)
        if result.get("success"):
            return result.get("fight", {})
        else:
            raise HTTPException(
                status_code=500, detail=result.get("error", "Unknown error")
            )
...
@app.post("/api/actions/fight/cancel")
def cancel_fight() -> Dict[str, Any]:
    try:
        from src.tasks import cancel_fight

        current_bot = get_bot()
        if not current_bot.logged_in:
            raise HTTPException(status_code=401, detail="Not logged in")
        result = cancel_fight(current_bot)
        if result.get("success"):
            invalidate_cache_pattern("dashboard")
            invalidate_cache_pattern("status")
            try:
                ...
                logger.warning(f"Failed to emit events after fight cancel: {e}")
        return result
    except HTTPException:
        raise
    except Exception as e:
